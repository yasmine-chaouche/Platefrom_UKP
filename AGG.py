...
def croisement_uniforme(parents, length):
    np.random.seed()
...
def geneticAlgorithm(items, capacity, nb_tour, solutions, nb_solutions, nb_per_group, proba_mutation, pc):
    ...
    for k in range(nb_tour):
        ...
        while (loop):
            ...
            # pp=np.random.rand()
            # if pp<pc:
            fils = croisement_uniforme([best_sol_1, best_sol_2], taille)
    ...
    return objects, solution, best_solution, gain_tot, poids


''''
#items= [[2,5],[3,2],[5,10],[7,20]]
#cap,nb,items = readData("Datasets\\Facile\\Moyenne\\cap591952_5000_facile.csv")
...
print("la solution: ",solution,"le gain: ",gain_tot,"\t temps : ",end-start)
...
# print("AG2-------------------------------------------------")
# start = time.time()
# objects,solution,gain_tot,poids =geneticAlgorithm(items,cap,20,100,2,0.05,0.6)
# #listn,sol,gainT=geneticAlgorithm(items,14,10,5,2,0.1)
# end = time.time()
# print("la solution: ",solution,"le gain: ",gain_tot,"\t temps : ",end-start)
'''
